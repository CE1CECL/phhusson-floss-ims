                    ...
                    while (bufPos < outBufInfo.size) {
                        ...
                        val rtpHeader =
                            listOf(
                                // RTP
                                0x80, // rtp version
                                (if (firstPacket) 0x80 else 0) or call.pcmaTrack, // payload type
                                (sequenceNumber shr 8),
                                (sequenceNumber and 0xff),
                                (timestamp shr 24),
                                ((timestamp shr 16) and 0xff),
...
a=sendrecv
                ...
                if (!isPrecondition && resp.statusCode == 183) {
                   ...
                   /* } else {
                        imsMediaManager.openSession(
                            ...
                            AudioConfig.Builder()
                                .setCodecType(AudioConfig.CODEC_PCMA)
                                .setRxPayloadTypeNumber(pcmaTrack.toByte())
                                .setTxPayloadTypeNumber(pcmaTrack.toByte())
                                .setRemoteRtpAddress(InetSocketAddress(rtpRemoteAddr, rtpRemotePort.toInt()))
                                .setSamplingRateKHz(8)
                                .setPcmaParams(PcmaParams.Builder()
                                    .setOctetAligned(false)
                                    .setPcmaMode(PcmaParams.PCMA_MODE_7)
                                    .build())
    ...
    fun callDecodeThread() {
        ...
        thread {
            ...
            while (true) {
                ...
                Rlog.d(TAG, "Received RTP data (expecting PCMA) ft is $ft")
    ...
    fun handleCall(request: SipRequest): Int {
        ...
        val pcmaTrackRequirements = trackRequirements(pcmaTrack)
